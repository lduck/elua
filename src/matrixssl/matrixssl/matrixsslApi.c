...
int32 matrixSslGetWritebuf(ssl_t *ssl, unsigned char **buf, uint32 requestedLen)
{
	...
	if (!(ssl->flags & SSL_FLAGS_SERVER)) {
		...
		if ((ssl->majVer == SSL3_MAJ_VER) && (ssl->minVer <= TLS_MIN_VER)
				&& (ssl->enBlockSize > 0) && (requestedLen > 1) &&
...
/*
	...
	to the SSL specification
*/
	if (requestedLen > SSL_MAX_PLAINTEXT_LEN) {
		requestedLen = SSL_MAX_PLAINTEXT_LEN;
	}
...
/*
	If the total overall size exceeds the maximum allowed record we need
	to pick a plaintext length that will keep us under our max.  Good value is
	the max amount minus the max overhead
*/

	if (requiredLen > SSL_MAX_BUF_SIZE) {
		overhead = matrixSslGetEncodedSize(ssl, 0) + ssl->enBlockSize;
		requiredLen = matrixSslGetEncodedSize(ssl, SSL_MAX_BUF_SIZE - overhead);
	}
...
*/
	    ...
	    /* if there is enough free space in buf, we need calculate different overhead */
            //requestedLen = sz - matrixSslGetEncodedOverheadForMaxBuf(ssl, sz);
            overhead = matrixSslGetEncodedBufOverhead(ssl, sz);
        }
...
/*
	Immediately reject a record that exceeds the max plaintext allowed by spec
*/
...
/*
	We just call matrixSslGetWritebuf ourselves to prepare the buffer
*/
	if ((rc = matrixSslGetWritebuf(ssl, &internalBuf, len)) < 0) {
		psTraceIntInfo("matrixSslEncodeToOutbuf failed allocating buffer: %d\n",
			rc);
		return rc;
	}
	psAssert((uint32)rc >= len);
	psAssert(ssl->outsize > 0 && ssl->outbuf != NULL);
