...
/*****************************************************************************/
int32 matrixSslGetEncodedBufOverhead(ssl_t *ssl, int32 sz)
{       
        int32 len = ssl->recordHeadLen;
        if (ssl->flags & SSL_FLAGS_WRITE_SECURE) {
                len += ssl->enMacSize;
#ifdef USE_TLS_1_1
        /*
...
#ifdef USE_BEAST_WORKAROUND
                if (ssl->bFlags & BFLAG_STOP_BEAST) { //FIXME: probably should not use psPadLenPwr2
                        /* Original message less one */
                        len += psPadLenPwr2(len - 1 - ssl->recordHeadLen, ssl->enBlockSize);
                        /* The single byte record overhead */
                        len += ssl->recordHeadLen + ssl->enMacSize;
                        ...
                        printf("\npred psPadLenPwr2() len: %d, sz:%d", len, sz);
            //len += psPadLenPwr2(len - ssl->recordHeadLen, ssl->enBlockSize);
            len += sz - psPadDownLenPwr2( sz - ssl->recordHeadLen, ssl->enBlockSize);
            printf("po psPadLenPwr2() len: %d, psPadLenPwr2:%d\n\n", len, psPadDownLenPwr2( sz - ssl->recordHeadLen, ssl->enBlockSize) );
            /* now is be padding 0, but return from psPadLenPwr2() is in that case ssl->enBlockSize */
            len++; /* now is padding 1 */
                }
#else
                //len += psPadLenPwr2(len - ssl->recordHeadLen, ssl->enBlockSize);
        len += sz - psPadDownLenPwr2( sz - ssl->recordHeadLen, ssl->enBlockSize);
...
*/
	...
	*c += sslWritePad(*c, (unsigned char)padLen);
	...
	if (pt == encryptStart) {
		/* In-situ encode */
		if ((rc = ssl->encrypt(&ssl->sec.encryptCtx, pt, encryptStart, 
		...
		*/
    	if (ssl->flags & SSL_FLAGS_WRITE_SECURE) {
			...
			if (divLen > 0) {
				rc = ssl->encrypt(&ssl->sec.encryptCtx, pt, encryptStart,
					divLen);
				if (rc < 0) {
					psTraceIntInfo("Error encrypting: %d\n", rc);
					return MATRIXSSL_ERROR;
			...
			rc = ssl->encrypt(&ssl->sec.encryptCtx, encryptStart + divLen,
