...
def get_user_includes(environ):
    ...
    def normalize(path):
        newpath = re_sconstruct_dir.split(path)[2]
...
def get_macros_from_cpp_defines(environ):
    ...
    macros = [macro for macro in environ['CPPDEFINES'] if macro
              is not None]
    return set(macros)
    ...
    for super_node in super_nodes:
        ...
        while node:
            if node and node.has_builder():
                ...
                if not compiler_environ and 'CC' in environ and 'CXX' \
                    in environ:
